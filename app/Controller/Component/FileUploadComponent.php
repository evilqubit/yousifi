			...
			if($file['error']!=0){
				...
				if($type=='image'){
					...
					if(!is_uploaded_file($file['tmp_name'])){
						$errorMsg='An error occured while uploading the file. Please try again.0';
						$error=true;
						$retArray=array('error'=>$error,'errorMsg'=>$errorMsg);
						return $retArray;
					}
						...
						if(!copy($file['tmp_name'],$fileDestination)){
							$errorMsg='An error occured while uploading the file. Please try again.1';
							$error=true;
							$retArray=array('error'=>$error,'errorMsg'=>$errorMsg);
							return $retArray;
						}
							...
							if($options['resize']){
								...
								foreach ($options['resizeOptions'] as $resize){
									if((isset($resize['folder']) && is_dir($resize['folder'])) && ((isset($resize['width']) && is_numeric($resize['width'])) || (isset($resize['height']) && is_numeric($resize['height'])))){
										...
										$command = $this->imageMagick."/convert -compress jpeg -quality 80 \"$fileDestination\" -scale '$resizeOpStart{$resize['width']}x{$resize['height']}$resizeOpEnd' \"$resizeSave\"";
										@passthru($command);
				...
				elseif ($type=='video'){
					if(!is_uploaded_file($file['tmp_name'])){
						$errorMsg='An error occured while uploading the file. Please try again.3';
						$error=true;
						$retArray=array('error'=>$error,'errorMsg'=>$errorMsg);
						return $retArray;
					}
						...
						if(!copy($file['tmp_name'],$fileDestination)){
							$errorMsg='An error occured while uploading the file. Please try again.4';
							$error=true;
							$retArray=array('error'=>$error,'errorMsg'=>$errorMsg);
							return $retArray;
						}
							...
							if($convert && file_exists($fileDestination)){
								...
								$saveTo=$flvPath.'/'.$baseFileName.'.mp4';

								//$cmd=$ffmpeg['ffmpegPath']."/ffmpeg -i $fileDestination -ar $ffmpeg[audioSamplingFrequency] -ab $ffmpeg[audioBitRate] -aspect $ffmpeg[videoAspectRatio]  -b $ffmpeg[videoBitRate] -r $ffmpeg[fps] -f mp4 -s $ffmpeg[videoDimensions] -ac $ffmpeg[audioChannels] -y $saveTo";

								$cmd="ffmpeg -i $fileDestination -acodec libfaac -ab 128k -ac 2 -r 25 -vcodec libx264 -vpre slow $saveTo";
								...
								$retArray['flvName']=$baseFileName.".mp4";
...
//
					...
					// debug ($file['tmp_name']);
					// debug (is_uploaded_file($file['tmp_name']));exit;
					// if(!is_uploaded_file($file['tmp_name'])){
					...
					// else{
						...
						if(!copy($file['tmp_name'],$fileDestination)){
							$errorMsg='An error occured while uploading the file. Please try again.7';
							$error=true;
							$retArray=array('error'=>$error,'errorMsg'=>$errorMsg);
							return $retArray;
						}
