 ...
 */
	public function setup(Model $Model, $config = array()) {
		...
		// for extracting the I18n fields
		
		if(isset($Model->actsAs["Translate"])){		
			
			$i18nModel = $Model->name."I18n";
			$Model->bindModel(array("hasMany"=>array("$i18nModel"=>array("foreignKey"=>"foreign_key"))));
			
		}

		
		$currModelsAliases = array();
		
		if(isset($Model->aliasesUsed))
			$currModelsAliases = $Model->aliasesUsed;
	
			
//		if(!empty($currModelsAliases)){
//			
//			
//			foreach ($currModelsAliases as $aliasName=>$aliasModelName){
//				if(!$this->modelsAliases[$aliasName]){
//					$this->modelsAliases[$aliasName] = $aliasModelName;
//				}
//			}
//		
//			foreach ($this->modelsAliases as $aliasName=>$aliasModelName){
//				$i18nModel = $aliasName."I18n";
//				
//				$newModelObj = ClassRegistry::init($aliasName);
//				$newModelObjI18n = ClassRegistry::init($i18nModel);
//				
//				$newModelObj->bindModel(array("hasMany"=>array("$i18nModel"=>array("className"=>$aliasModelName,"foreignKey"=>"foreign_key"))));
//			}
//		}
//		
