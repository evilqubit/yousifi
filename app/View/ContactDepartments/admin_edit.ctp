...
<form id="PageForm" type='file'  enctype="multipart/form-data"  class="form-horizontal" accept-charset="utf-8" method="post" action="/admin/<?=$controllerName?>/edit/<?=$id?>" novalidate="novalidate">
    ...
    <div class="box">
        ...
        <div class="box-content">
			...
			?>
       		...
       		<div class="control-group">
				<label class="control-label" for="JobPublish">email</label>
				<div class="input_div" style='float:left' ><?php echo $this->Form->input("$modelName.email",array('class'=>'email required', 'type'=>'text', 'escape' => false,'label'=>false));?></div>
			</div>
			...
			<div class="tab-pane active" id="tab-1-1">
	       	...
	       	<!-- image 1  -->
			<!-- <div class="control-group">
				<label class="control-label" for="JobPublish">Mobile_image</label>
					<div class="input_div" style='float:left' ><?php echo $this->Form->input("$modelName.mobile_image",array('label'=>false,"type"=>"file","size"=>"43"));?>
						<div> Preferred size : <?=$preferred_size_mobile?></div>
						<div class="clear_btn input_row" onclick="clear_img_path(this);">Clear Image Path</div>
					</div>
				
				<?php if(isset($this->request->data[$modelName]["mobile_image"]) && !empty($this->request->data[$modelName]["mobile_image"])){ ?>
					<div class="input_row " style='float:left' >
						<div class="label">Chosen Image</div>
						<div class="input_div"><a href="/files/<?=$userFilesFolder?>/original/<?php echo $this->request->data["$modelName"]["mobile_image"];?>" class='lightbox' ><img src="/files/<?=$userFilesFolder?>/thumb/<?php echo $this->request->data["$modelName"]["mobile_image"];?>" alt="" border="0" /></a></div>		
					</div>
				<?php } ?>
       		</div> -->		
