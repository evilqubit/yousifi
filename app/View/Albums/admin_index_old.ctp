...
<div class="subtitle">List Albums</div>

<?php 

//print_r($paginator);exit;
$add_location="/admin/$controllerName/add/$type/";
...
<button class="FloatClass submit_btn" style="clear:both;margin-bottom:10px;" onclick="window.location='/admin/<?=$controllerName?>/index/<?=$type?>/1'">Order all entries</button>
	<table border="0" width="500" cellpadding="1" cellspacing="1" class="FloatClass" id="sortable_list">
			...
			if(!empty($data)){
				...
				foreach ($data as $output)
				{
					...
					$order_link=$this->html->link($image8, $order_location.$output_id, array('escape'=>false), null, false);
					$order_div='<div class="FloatClass" id="order_div_'.$output_id.'">'.$order_link.'</div>';
...
<div style="margin:20px 0px 20px 0px;width:560px;text-align:left; border:solid 1px #72BF44;padding:10px 0px 10px 10px;height:auto;overflow:hidden;clear:both;float:left;">
	<div style="float:left;clear:both;margin-bottom:10px;">
		<div style="float:left;"><img src="/img/admin/edit.gif" /></div>
	...
	<div style="float:left;">
		<div style="float:left;"><img src="/img/admin/arrow.png" /></div>
